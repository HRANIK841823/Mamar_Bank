...
class UserRegistrationFrom(UserCreationForm):
        ...
        class Meta:
                model=User
                fields=['username','first_name','last_name','email','password1','password2','birth_date','gender','street_address','city','postal_code','country','account_type']
        
        def save(self,commit=True):
                our_user=super().save(commit=False) #ai line er mani ami akhon database a data save korbo na
                if commit==True:
...
class TransferAmountForm(forms.Form):
    receiver_account_no = forms.IntegerField(label="Rec Account Number")
    amount = forms.DecimalField(max_digits=12, decimal_places=2, label="Amount to Transfer")
